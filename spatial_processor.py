...
def cal_attn_mask_1_4(total_length, id_length, sa32, sa64, height, width, device="cuda", dtype=torch.float16):
    nums_32 = (height // 32) * (width // 32)
    nums_64 = (height // 64) * (width // 64)  # Corrected to 64
    bool_matrix32 = torch.rand((1, total_length * nums_32), device=device, dtype=dtype) < sa32
    bool_matrix64 = torch.rand((1, total_length * nums_64), device=device, dtype=dtype) < sa64
    bool_matrix32 = bool_matrix32.repeat(total_length, 1)
    bool_matrix64 = bool_matrix64.repeat(total_length, 1)
    for i in range(total_length):
        bool_matrix32[i:i + 1, id_length * nums_32:] = False
        bool_matrix64[i:i + 1, id_length * nums_64:] = False
        bool_matrix32[i:i + 1, i * nums_32:(i + 1) * nums_32] = True
        bool_matrix64[i:i + 1, i * nums_64:(i + 1) * nums_64] = True
    mask32 = bool_matrix32.unsqueeze(1).repeat(1, nums_32, 1).reshape(-1, total_length * nums_32)
    mask64 = bool_matrix64.unsqueeze(1).repeat(1, nums_64, 1).reshape(-1, total_length * nums_64)
    wandb.log({"mask32": wandb.Image(mask32.float().cpu().numpy())})
    wandb.log({"mask64": wandb.Image(mask64.float().cpu().numpy())})
    return mask32, mask64
...
class AttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        #     # print(f"Final hidden_states shape: {hidden_states.shape}")
        #     heatmaps = hidden_states.cpu().numpy().mean(axis=2)  # Shape: [8, 1024]
...
class SpatialAttnProcessor2_0(torch.nn.Module):
    ...
    """
    def __init__(self, hidden_size=None, cross_attention_dim=None, id_length=4, total_count=0, sa32=0.5, sa64=0.5, height=512, width=512, attn_count=0, curr_step=0, write=False, device="cuda", dtype=torch.float16):
        super().__init__()
        self.device = device
        self.dtype = dtype
        ...
        self.sa32 = sa32
        self.sa64 = sa64
        self.mask32, self.mask64 = cal_attn_mask_1_4(
    ...
    def __call__(
            ...
            if random_number > rand_num:
                if not self.write:
                    if hidden_states.shape[1] == (self.height // 32) * (self.width // 32):
                        attention_mask = self.mask32[self.mask32.shape[0] // self.total_length * self.id_length:]
                    else:
                        attention_mask = self.mask64[self.mask64.shape[0] // self.total_length * self.id_length:]
                else:
                    if hidden_states.shape[1] == (self.height // 32) * (self.width // 32):
                        attention_mask = self.mask32[:self.mask32.shape[0] // self.total_length * self.id_length, :self.mask32.shape[0] // self.total_length * self.id_length]
                    else:
                        attention_mask = self.mask64[:self.mask64.shape[0] // self.total_length * self.id_length, :self.mask64.shape[0] // self.total_length * self.id_length]
        ...
        if self.attn_count == self.total_count:
            self.attn_count = 0
            self.cur_step += 1
            self.mask32, self.mask64 = cal_attn_mask_1_4(self.total_length, self.id_length, self.sa32, self.sa64, self.height, self.width, device=self.device, dtype=self.dtype)
