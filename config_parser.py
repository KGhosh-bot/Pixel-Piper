...
def parse_arguments():
    ...
    parser.add_argument("--prompt_file", type=str, default="prompt_sets.txt", help="Path to the prompt file.")
    ...
    parser.add_argument("--seed", nargs='+', type=int, default=[3,4,80,55364,212,998], help="Which random seeds to use when generating")
    ...
    parser.add_argument("--attention_corr_indices", type=str, default='[[[2,3],[6,7]], [[2,3]], [[2,3]], [[2,3]], [[2,3], [7,8]], [[2,3]]]', help="JSON string of corr inclusion pair")
    parser.add_argument("--attention_exist_indices", type=str, default='[[3,7], [3,5], [3,6], [3,6], [3,8], [3,5]]', help="JSON string of exist inclusion pair")
    parser.add_argument("--attention_leak_indices", type=str, default='[[[2,7], [6,3]], [], [], [], [[2,8], [7,3]], []]', help="JSON string of leak inclusion pair")
    parser.add_argument("--attention_possession_indices", type=str, default='[[], [[3,5]], [[3,6]], [[3,6]], [], []]', help="JSON string of semantic inclusion pair")
    ...
    parser.add_argument("--model_name", type=str, default="Stable", help="Model name from models_dict")
    ...
    # Parse JSON strings
    args.attention_corr_indices = json.loads(args.attention_corr_indices)
    args.attention_exist_indices = json.loads(args.attention_exist_indices)
    args.attention_leak_indices = json.loads(args.attention_leak_indices)
    args.attention_possession_indices = json.loads(args.attention_possession_indices)
    return args
